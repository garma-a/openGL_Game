...
float assistantY = 0.3f;
float assistantLastShotTime = 0.0f;
const float assistantShotInterval = 1.0f;
const int assistantCost = 50;
...
void drawShop() {
  ...
  if (hasAssistant) {
    renderText(-0.45f, -0.55f, "Assistant already hired!");
  } else {
    renderText(-0.45f, -0.55f, "Hire helper to shoot zombies (50 coins)");
    renderText(-0.45f, -0.65f, "Press A to hire assistant");
  }
  // Close shop hint
...
void loadZombies(int count) {
  ...
  for (int i = 0; i < count; ++i) {
    ...
    z.y = z.baseY;
    z.angle = 0.0f;
    z.zigzagEnabled = (currentDifficulty == DIFFICULTY_HARD);
    z.isAlive = true;
    if (level >= 10) {
      z.health = 5;
    } else if (level >= 4) {
...
void display() {
  ...
  // Draw assistant if owned
  if (hasAssistant) {
    glPushMatrix();
    glTranslatef(0.8f, assistantY, 0.0f);
    glScalef(0.15f, 0.15f, 1.0f);
    drawAssistant();
    glPopMatrix();
  }
...
void purchaseAssistant() {
  if (!hasAssistant && points >= assistantCost) {
...
void update(int value) {
  ...
  // Assistant shooting logic
  if (hasAssistant &&
      currentTime - assistantLastShotTime >= assistantShotInterval) {
    // Find closest zombie
    float closestDist = 999.0f;
    Zombie *targetZombie = nullptr;
    for (auto &zombie : zombies) {
      if (zombie.isAlive && zombie.x < 0.7f) {
        float dist = 0.8f - zombie.x;
        if (dist < closestDist) {
          closestDist = dist;
          targetZombie = &zombie;
        }
    ...
    if (targetZombie != nullptr) {
      // Move assistant toward target Y
      float targetY = targetZombie->y;
      if (assistantY < targetY - 0.05f) {
        assistantY += 0.02f;
      } else if (assistantY > targetY + 0.05f) {
        assistantY -= 0.02f;
      }
      // Shoot at zombie
      Bullet b;
      ...
      b.b = 0.5f;
      bullets.push_back(b);
      assistantLastShotTime = currentTime;
    }
  }
  ...
  for (auto &zombie : zombies) {
    if (zombie.isAlive) {
      ...
      if (zombie.x >= serverLeft) {
        ...
        // Shield reduces damage by 50% when active
        if (upgradeEndTime > 0.0f) {
          damage = damage / 2;
        }
        playerHealth -= damage;
          ...
          if (bullet.x >= zombie.x - 0.024f && bullet.x <= zombie.x + 0.024f &&
            ...
            if (zombie.health <= 0) {
              ...
              if (roll <= healthPickupDropChance) {
                ...
                pickup.y = zombie.y;
                pickup.active = true;
                pickup.healAmount = healthPickupHeal;
...
void initSound() {
  ...
  // Load sound effects (create a sounds folder with these files)
  // Note: Mix_LoadWAV can load WAV, AIFF, RIFF, OGG, and VOC formats
  // For MP3 files, we need to use Mix_LoadMUS or convert them to WAV
  shootSound = Mix_LoadWAV("sounds/shoot.wav");
  if (!shootSound) {
    printf("Warning: Could not load shoot.wav: %s\n", Mix_GetError());
  }

  // Try WAV first, then MP3 for each sound
  zombieDeathSound = Mix_LoadWAV("sounds/zombie_death.wav");
  if (!zombieDeathSound) {
    // SDL_mixer with proper codec support can load MP3 as chunks too
  ...
  gameOverSound = Mix_LoadWAV("sounds/game_over.wav");
  if (!gameOverSound) {
    gameOverSound = Mix_LoadWAV("sounds/game_over.mp3");
  }
  if (!gameOverSound) {
    printf("Warning: Could not load game_over sound: %s\n", Mix_GetError());
  }
  ...
  if (!hitSound) {
    hitSound = Mix_LoadWAV("sounds/hit.mp3");
  }
  if (!hitSound) {
    printf("Warning: Could not load hit sound: %s\n", Mix_GetError());
  }

  // Load background music (optional)
  backgroundMusic = Mix_LoadMUS("sounds/background.mp3");
  if (!backgroundMusic) {
    backgroundMusic = Mix_LoadMUS("sounds/background.wav");
  }
  if (!backgroundMusic) {
    printf("Note: No background music found (optional)\n");
  }
